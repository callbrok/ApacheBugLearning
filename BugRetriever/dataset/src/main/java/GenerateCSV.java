...
public class GenerateCSV {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int k=0; k<finalListRelease.size()-1; k++){
            ...
            arff.buildArff(csv.buildCSV(gtc.retrieveAllGitDataSet(PROJECT, String.valueOf(k)), finalListRelease.get(k).getReleaseFromJira().getName(), k + "_TRAINING"));

            // Build Testing Set Dataset with k+1 Release
            arff.buildArff(csv.buildCSV(List.of(finalListRelease.get(k+1)), finalListRelease.get(k+1).getReleaseFromJira().getName(), k + "_TESTING"));
        }
