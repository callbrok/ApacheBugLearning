...
public class MetricsRetriever {
    ...
    public Metrics metricsHelper(ReleaseTag taggedReleaseToGetFileMetrics, ReleaseTag previousTaggedReleaseToGetFileMetrics, Boolean isFirst, String pathOfFile, List<Commit> relatedCommitsOfCurrentTaggedRelease) throws Exception {

        // Retrieve LOC Metric -------------------------
        List<Integer> locMetricAndNMethods = locAndMethodsNumberMetrics(taggedReleaseToGetFileMetrics, taggedReleaseToGetFileMetrics.getCurrentRepo().getjGitRepository().findRef("HEAD").getObjectId(), pathOfFile, false);
        ...
        List<Integer> locMetrics = locOtherMetrics(taggedReleaseToGetFileMetrics, previousTaggedReleaseToGetFileMetrics, pathOfFile, isFirst, relatedCommitsOfCurrentTaggedRelease);
    ...
    private List<Integer> locOtherMetrics(ReleaseTag taggedReleaseToGetFileMetrics, ReleaseTag previousTaggedReleaseToGetFileMetrics, String pathOfFile, Boolean isFirst, List<Commit> relatedCommitsOfCurrentTaggedRelease) throws Exception {
        // LOC Touched:
        ...
        for(int k=0; k<relatedCommitsOfCurrentTaggedRelease.size(); k++){
            ...
            if(k==0 && !isFirst){
                parent = takeLatestCommitOfTag(previousTaggedReleaseToGetFileMetrics, pathOfFile);
            ...
            //      the file is just added
                ...
                int lineFirstCommit = locAndMethodsNumberMetrics(taggedReleaseToGetFileMetrics, commitId, pathOfFile, true).get(0);
            ...
            for (DiffEntry diff : diffs) {
                if(diff.getNewPath().equals(pathOfFile)){
