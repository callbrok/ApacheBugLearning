...
public class ReleaseRetriever {
    ...
    public List<Release> getReleaseFromProject(String projectName, Boolean getHalfReleases, String releaseRange) throws IOException, ParseException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            // Check if not released and Check if "releaseDate" field exist in Json, (like STORM field n.5), so skip
            if(!explrObject.getBoolean("released") || !explrObject.has("releaseDate")){ continue; }
        ...
        if(Boolean.TRUE.equals(getHalfReleases)){releases = setReleaseIndex(releases.subList(0, (releases.size()-1)/2));}
